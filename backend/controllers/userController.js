...
/**
 * @desc    Authenticates user and gets token
...
/**
 * @desc    Registers user & receives token
...
const getUserProfileById = asyncHandler(async (req, res) => {
    ...
    if (user && (user.isAdmin || id === user._id.toString())) {
        res.status(200)
        res.json({
            _id: user._id,
            name: user.name,
        ...
        res.status(404)
        throw new Error('User not found or not authorized to view')
    }
})
...
const updateUserById = asyncHandler(async (req, res) => {
    ...
    if (user) {
        user.name = newInfo.name || user.name
        user.email = newInfo.email || user.email
        if ("isAdmin" in newInfo) {
            user.isAdmin = newInfo.isAdmin
        }
